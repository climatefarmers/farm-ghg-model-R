get_leakage <- function(inputs) {
  ...
  # this used for the overall ERR calculations
  # note that leakage is always positive (i.e., you can't earn credits from reducing OM imports)
  leakage_diffs <- leakage_annual %>%
    filter(year_index >= 0) %>% 
    mutate(CO2eq_leakage_diff = pmax(0, CO2eq_leakage - leakage_baseline$CO2eq_leakage))
